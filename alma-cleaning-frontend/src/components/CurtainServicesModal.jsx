...
const CurtainServicesModal = ({ isOpen, onClose, onAddToMainCart, isMobile, isSmallMobile }) => {
  const [cartItems, setCartItems] = useState([]);

  const services = [
    { id: 'curtain-cleaning', title: 'Химчистка штор', price: 1500, image: cover, step: 0.5, min: 1 },
    { id: 'tulle-cleaning', title: 'Химчистка тюли', price: 1000, image: cover, step: 0.5, min: 1 }
  ];
        ...
        <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: '20px', flex: 1, overflow: 'hidden' }}>
          <div style={{ flex: isMobile ? '1' : '2', overflowY: 'auto', paddingRight: isMobile ? '5px' : '10px' }}>
            <h2 style={{
              ...
              Химчистка штор и тюли (цена за погонный метр)
            </h2>
            <div style={{
              display: 'grid',
              ...
              {services.map(service => {
                ...
                return (
                  ...
                  >
                    <img src={service.image} alt={service.title} style={{ width: isSmallMobile ? '60px' : '80px', height: isSmallMobile ? '60px' : '80px', borderRadius: '8px', objectFit: 'cover' }} />
                    <div style={{ flex: 1 }}>
                      <h3 style={{ fontSize: isSmallMobile ? '1rem' : '1.1rem', color: '#fff', margin: 0, marginBottom: '5px' }}>{service.title}</h3>
                      <p style={{ fontSize: isSmallMobile ? '0.9rem' : '1rem', color: '#f47ac2', margin: 0, fontWeight: 'bold' }}>{service.price.toLocaleString()} тг/п.м.</p>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: isSmallMobile ? '6px' : '10px', background: 'rgba(0,0,0,0.2)', padding: '5px 8px', borderRadius: '8px' }}>
                      <button onClick={() => handleQuantityChange(service.id, quantity - service.step)} style={{ background: 'none', border: 'none', color: '#fff', cursor: 'pointer', fontSize: isSmallMobile ? '1rem' : '1.2rem' }}><FaMinus /></button>
                      <input
                        type="number"
                        value={quantity}
                        ...
                        step={service.step || 1}
                      />
                      <button onClick={() => handleQuantityChange(service.id, quantity + service.step)} style={{ background: 'none', border: 'none', color: '#fff', cursor: 'pointer', fontSize: isSmallMobile ? '1rem' : '1.2rem' }}><FaPlus /></button>
                    </div>
                  </motion.div>
                );
              })}
                    ...
                    {cartItems.map(item => (
                      <li key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px', background: 'rgba(255,255,255,0.05)', padding: '8px 10px', borderRadius: '8px' }}>
                        <div>
                          ...
                          <p style={{ margin: '4px 0 0', color: '#ccc', fontSize: isMobile ? '0.8rem' : '0.9rem' }}>{item.quantity} п.м. x {item.price.toLocaleString()} тг</p>
                        </div>
...
export default CurtainServicesModal; 